...
public partial class StudentManagementView : UserControl
{
    ...
    private static class ElementIdentifiers
    {
        ...
        public static readonly string[] ActionButtons = { "SearchButton", "ReportButton" };
    }
    ...
    private void UpdateContentGrid(Grid grid, LayoutConfiguration config)
    {
        ...
        {
            // Mobile: Stack student list and details vertically
            grid.ColumnDefinitions.Add(new ColumnDefinition(GridLength.Star));
            grid.RowDefinitions.Add(new RowDefinition(GridLength.Star));
            grid.RowDefinitions.Add(new RowDefinition(GridLength.Auto));

            if (_elementCache.TryGetElement<Border>(ElementIdentifiers.StudentListCard, out Border? listCard) && listCard != null)
            {
            ...
            if (_elementCache.TryGetElement<Border>(ElementIdentifiers.StudentDetailsCard, out Border? detailsCard) && detailsCard != null)
            {
                Grid.SetColumn(detailsCard, 0);
                Grid.SetRow(detailsCard, 1);
                detailsCard.Margin = new Thickness(0, 12, 0, 0);
            }
        }
        else
        {
            ...
            if (_elementCache.TryGetElement<Border>(ElementIdentifiers.StudentDetailsCard, out Border? detailsCard) && detailsCard != null)
            {
                ...
                detailsCard.Margin = new Thickness(12, 0, 0, 0);
            }
        }
    }
    ...
    {
        if (!_elementCache.TryGetElement<ScrollViewer>(ElementIdentifiers.StudentListScrollViewer, out ScrollViewer? scrollViewer) || scrollViewer == null)
            return;
    ...
    private void ApplyDynamicElementStyles(ResponsiveState state)
    {
        ...
        {
            if (border.Classes.Contains("card") || border.Classes.Contains("student-card"))
            {
