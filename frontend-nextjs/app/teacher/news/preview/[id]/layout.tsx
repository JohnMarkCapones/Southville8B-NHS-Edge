...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    if (!article) {
      return {
        title: "News Article Preview | News",
        description: "Preview news article from Southville 8B National High School",
        robots: { index: false, follow: false },
      }
    }

    const path = `/teacher/news/preview/${article.id}`
    const fullUrl = absoluteUrl(path)
    const title = `${article.title} (Preview) | Southville 8B NHS News`
    ...
    return {
      ...
      robots: { index: false, follow: false }, // Preview pages shouldn't be indexed
      openGraph: {
        type: 'article',
        title,
        description,
    ...
    return {
      title: "News Article Preview | Southville 8B NHS",
      description: "Preview news article from Southville 8B National High School",
      robots: { index: false, follow: false },
    }
  }
...
export default function TeacherNewsPreviewLayout({
  children,
}: {
