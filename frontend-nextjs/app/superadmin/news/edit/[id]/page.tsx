...
import { useUpdateNews } from "@/hooks/useNewsMutations"
import type { UpdateNewsDto } from "@/lib/api/endpoints/news"
...
export default function EditArticlePage() {
  ...
  useEffect(() => {
    const fetchArticleData = async () => {
      ...
      try {
        ...
        const article = await newsApi.getNewsById(articleId)

        if (!article) {
          toast({
            title: "⚠️ Article Not Found",
        ...
        // Map the article data to form data
        setFormData({
          title: article.title || "",
          description: article.excerpt || "",
          articleHtml: article.content || "",
          articleJson: null, // Will need to fetch from API if available
          categoryId: typeof article.category === "object" ? article.category.id : "",
          tags: article.tags || [],
          featuredImageUrl: article.image || "",
          visibility: mapFrontendVisibilityToBackend(article.visibility || "Public"),
          scheduledDate: article.publishedDate || "",
          coAuthorIds: [],
          slug: article.slug || "",
        ...
        console.error("[EditArticlePage] Error fetching article:", error)
        toast({
          title: "⚠️ Failed to Load Article",
          description: "Could not load the article data. Please try again.",
          variant: "destructive",
        })
      } finally {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        console.error("[EditArticlePage] Error fetching categories:", error)
        toast({
          title: "⚠️ Failed to load categories",
          description: "Categories could not be loaded. You can still edit the article without a category.",
          variant: "destructive",
        })
      } finally {
  ...
  // Helper function to map frontend visibility to backend format
  const mapFrontendVisibilityToBackend = (
    visibility: string
  ): "public" | "students" | "teachers" | "private" => {
    const visibilityMap: Record<string, "public" | "students" | "teachers" | "private"> = {
      Public: "public",
  ...
  const addCoAuthor = () => {
    if (currentCoAuthor.trim() && !formData.coAuthorIds.includes(currentCoAuthor.trim())) {
      setFormData((prev) => ({
        ...prev,
        coAuthorIds: [...prev.coAuthorIds, currentCoAuthor.trim()],
      }))
      setCurrentCoAuthor("")
    }
  }
  ...
  const removeCoAuthor = (authorToRemove: string) => {
    setFormData((prev) => ({
      ...prev,
      coAuthorIds: prev.coAuthorIds.filter((author) => author !== authorToRemove),
    }))
  }
  ...
  const performSave = async (saveAsDraft: boolean = true) => {
    try {
      ...
      const updateDto: UpdateNewsDto = {
        ...
        article_json: formData.articleJson,
        article_html: formData.articleHtml,
      ...
      // Call the update API
      const updatedArticle = await updateNews({ id: articleId, data: updateDto })
      ...
      if (saveAsDraft) {
        toast({
          ...
          description: (
            <div className="space-y-2">
              <p className="font-medium text-foreground">"{updatedArticle.title}" has been saved as a draft.</p>
        ...
        toast({
          ...
          description: (
            <div className="space-y-2">
              <p className="font-medium text-foreground">"{updatedArticle.title}" has been updated and saved.</p>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <div className="w-4 h-4 bg-gradient-to-br from-blue-500 to-purple-600 rounded flex items-center justify-center text-white text-xs font-bold">
                  <FileText className="w-3 h-3" />
                </div>
      ...
      toast({
        ...
        description: error?.message || "An error occurred while updating the article. Please try again.",
        variant: "destructive",
        duration: 5000,
      })
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-950 dark:via-blue-950 dark:to-indigo-950 flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Loading Article...</h2>
            <p className="text-gray-600 dark:text-gray-400 mt-2">Please wait while we fetch the article data</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="container mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white transition-colors">Edit Article</h1>
              <div className="flex items-center gap-4 mt-1">
                <p className="text-gray-600 dark:text-gray-400">Update and manage your news article</p>
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "edit" | "preview")} className="w-full">
          <TabsList className="grid w-full max-w-md mx-auto mb-8 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm">
                    ...
                    <div className="space-y-2">
                      ...
                      {formData.coAuthorIds.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2">
                          {formData.coAuthorIds.map((author) => (
                            <Badge
                              key={author}
                              variant="secondary"
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent className="dark:bg-slate-800 dark:border-slate-700">
                          <SelectItem value="draft" className="dark:text-white dark:focus:bg-slate-700">
                            Draft
                          </SelectItem>
                          <SelectItem value="scheduled" className="dark:text-white dark:focus:bg-slate-700">
                            Scheduled
                          </SelectItem>
                          <SelectItem value="published" className="dark:text-white dark:focus:bg-slate-700">
                            Published
                          </SelectItem>
                          <SelectItem value="archived" className="dark:text-white dark:focus:bg-slate-700">
                            Archived
                          </SelectItem>
                        </SelectContent>
                      </Select>
                  ...
                  <CardContent className="space-y-4">
                    <div className="space-y-2">
                      ...
                      {categories.length === 0 && !isLoadingCategories && (
                        <p className="text-xs text-gray-500 dark:text-gray-400">
                          No categories available. You can still edit the article without a category.
                        </p>
                      )}
                    </div>
              ...
              <Card className="shadow-lg border-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm transition-all duration-300 hover:shadow-xl">
                ...
                <CardContent>
                  <TiptapEditor
                    ...
                    onChange={(html, json) =>
                      setFormData((prev) => ({ ...prev, articleHtml: html, articleJson: json }))
                    }
                  />
                </CardContent>
              </Card>
                ...
                {formData.coAuthorIds.length > 0 && (
                  <div className="mb-8 border-l-4 border-primary pl-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {formData.coAuthorIds.map((authorId) => (
                        <Badge key={authorId} variant="secondary" className="break-words max-w-full">
                          <UserCircle className="w-3 h-3 mr-1 flex-shrink-0" />
                          <span className="break-words">{authorId}</span>
                        </Badge>
                      ))}
                    </div>
                  </div>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-3 text-base">
              <p className="text-slate-700 dark:text-slate-300">
                This article doesn't have any images. Articles with images perform better and are more engaging for
                readers.
              </p>
              <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
                <p className="text-sm text-blue-900 dark:text-blue-100 font-medium mb-1">Best Practices:</p>
