...
import { AttemptWarningModal } from "./attempt-warning-modal";
import { LockedLoginButton } from "./locked-login-button";
...
export function EnhancedPortalLogin() {
  const [showPassword, setShowPassword] = useState(false);
  const [loginError, setLoginError] = useState<string | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [showWarningModal, setShowWarningModal] = useState(false);
  const [showAttemptWarningModal, setShowAttemptWarningModal] = useState(false);
  ...
  // Progressive rate limiting hook
  const {
    attemptCount,
    isLockedOut,
    remainingTime,
    ...
    resetAttempts,
  } = useProgressiveRateLimit(`portal-${role}`);
  ...
  const roleConfig = {
    student: {
      ...
      features: ["View Grades", "Learning Material Access", "Class Schedule", "Quiz Access", "Study Tools"],
  ...
  const onSubmit = async (data: LoginFormInputs) => {
    ...
    if (isLockedOut) {
      setLoginError(
        `Account is temporarily locked. Please try again in ${Math.ceil(retryAfterSeconds / 60)} minute(s).`
      );
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        // Show modal based on current attempt count + 1 (since we just recorded a failed attempt)
        const newAttemptCount = attemptCount + 1;
        console.log('🔍 Modal Debug:', { attemptCount, newAttemptCount, isLockedOut });
        
        if (newAttemptCount < 5) {
          console.log('📢 Showing attempt warning modal');
          setShowAttemptWarningModal(true);
        } else {
          console.log('🔒 Showing lockout warning modal');
      ...
      // Show modal based on current attempt count + 1 (since we just recorded a failed attempt)
      const newAttemptCount = attemptCount + 1;
      console.log('🔍 Modal Debug (catch):', { attemptCount, newAttemptCount, isLockedOut });
      
      if (newAttemptCount < 5) {
        console.log('📢 Showing attempt warning modal (catch)');
        setShowAttemptWarningModal(true);
      } else {
        console.log('🔒 Showing lockout warning modal (catch)');
        setShowWarningModal(true);
      }
    } finally {
  ...
  return (
    ...
    >
          ...
          >
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              <CardFooter className="flex flex-col gap-4 pt-6">
                {/* Enhanced Login Button with Lockout */}
                <LockedLoginButton
                  ...
                  className={cn(
                    "h-12 text-lg font-semibold shadow-lg hover:shadow-xl transition-all duration-300",
                    !isLockedOut && isGamingTheme
      ...
      <AttemptWarningModal
        isOpen={showAttemptWarningModal}
        onClose={() => setShowAttemptWarningModal(false)}
        attemptCount={attemptCount}
        isGamingTheme={isGamingTheme}
      />
    </div>
