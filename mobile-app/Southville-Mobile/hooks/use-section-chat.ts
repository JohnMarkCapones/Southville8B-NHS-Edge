...
export interface UseSectionChatReturn {
	...
	participantsMap: Map<string, { full_name: string; email?: string; role?: string }>;
}

function calculateUnread(messages: ChatMessage[], lastReadIso?: string | null): number {
...
async function getSupabaseAuthToken(tokens: { accessToken: string } | null): Promise<string | null> {
	// First, try to use the stored token from auth session (same token used for API calls)
	if (tokens?.accessToken) {
		if (__DEV__) {
			console.log("[useSectionChat] Using stored auth token:", `${tokens.accessToken.substring(0, 20)}...`);
		}
	...
	if (!supabase) {
		if (__DEV__) console.warn("[useSectionChat] Supabase not available for auth token");
		return null;
	}
	try {
		const { data: session, error } = await supabase.auth.getSession();
		if (error) {
			if (__DEV__) console.warn("[useSectionChat] Failed to get Supabase session:", error.message);
			return null;
		}
		const token = session?.session?.access_token || null;
		if (__DEV__) {
			console.log("[useSectionChat] Token from Supabase session:", token ? `${token.substring(0, 20)}...` : "null");
		}
		return token;
	} catch (e) {
		if (__DEV__) console.warn("[useSectionChat] Error getting Supabase session:", e);
		return null;
	}
}

export function useSectionChat(options?: UseSectionChatOptions): UseSectionChatReturn {
	...
	const participantsMapRef = useRef<Map<string, { full_name: string; email?: string; role?: string }>>(new Map());
	...
	useEffect(() => {
		...
		if (supabase && tokens?.accessToken) {
			supabase.auth.setSession({
				...
				if (__DEV__) console.warn("[useSectionChat] Failed to set Supabase session:", e);
			});
		}
		...
		async function bootstrap() {
			...
			if (hasBootstrappedRef.current) {
				if (__DEV__) console.log("[useSectionChat] Bootstrap already ran, skipping");
				return;
			}
			...
			try {
				...
				if (__DEV__) {
					console.log("[useSectionChat] bootstrap sectionId:", sectionId, "user:", user?.id, "student:", !!user?.student);
				}
				if (!sectionId) {
					if (__DEV__) console.warn("[useSectionChat] No section_id found for user");
				...
				const participantsMap = new Map<string, { full_name: string; email?: string; role?: string }>();
				...
				if (chatServiceBase) {
					...
					try {
						...
						if (authToken) {
							...
							if (__DEV__) console.log("[useSectionChat] Including auth token in GET request");
						} else {
							if (__DEV__) console.warn("[useSectionChat] No auth token available for GET request");
						}
						...
						if (convResp.ok) {
							...
							if (__DEV__) {
								console.log("[useSectionChat] conversations resp status:", convResp.status, !!convData);
								console.log("[useSectionChat] conversations response:", JSON.stringify(convData, null, 2).substring(0, 500));
							}
							// API returns { conversations: [...], total, page, limit }
							const conversations = Array.isArray(convData?.conversations) 
								? convData.conversations 
								: (Array.isArray(convData?.data) ? convData.data : (Array.isArray(convData) ? convData : []));
							
							if (__DEV__) {
								console.log("[useSectionChat] Parsed conversations count:", conversations.length);
								console.log("[useSectionChat] Conversation types:", conversations.map((c: any) => ({ id: c.id, type: c.type, title: c.title })));
							}
							
							// Filter for group_section conversations
							const sectionConv = conversations.find((c: any) => c.type === "group_section");
							resolvedConvId = sectionConv?.id ?? null;
							
							// Extract participant user info for sender names and roles
							if (sectionConv?.participants && Array.isArray(sectionConv.participants)) {
								...
								if (__DEV__) {
									console.log("[useSectionChat] Extracted participants map:", Array.from(participantsMap.entries()));
								}
							}
							
							if (resolvedConvId && __DEV__) {
								console.log("[useSectionChat] Found section conversation from API:", resolvedConvId, sectionConv.title);
							} else if (__DEV__) {
								console.warn("[useSectionChat] No group_section conversation found in API response");
							}
						} else {
							const fallbackText = await convResp.text().catch(() => "");
							if (__DEV__) console.warn("[useSectionChat] conversations non-200:", convResp.status, fallbackText?.slice(0, 200));
						}
					} catch (e) {
						if (__DEV__) console.warn("[useSectionChat] Chat service lookup failed, using Supabase:", e);
					}
				}
				...
				if (!resolvedConvId && supabase && user?.id) {
					if (__DEV__) console.log("[useSectionChat] Looking up user's conversations via conversation_participants");
					try {
						...
						if (!participantsError && participantsData && participantsData.length > 0) {
							// Filter for group_section conversations
							const sectionConvs = participantsData.filter((p: any) => 
								p.conversations && p.conversations.type === "group_section"
							);
							
							if (__DEV__) {
								console.log("[useSectionChat] Filtered section conversations:", sectionConvs.length);
							}
							
							if (sectionConvs.length > 0) {
								// Use the first group_section conversation found
								resolvedConvId = sectionConvs[0].conversation_id;
								if (__DEV__) console.log("[useSectionChat] Found conversation via participants:", resolvedConvId);
							} else if (__DEV__) {
								console.log("[useSectionChat] User has conversations but none are group_section type");
								console.log("[useSectionChat] All conversation types:", participantsData.map((p: any) => p.conversations?.type));
							}
						} else if (participantsError && __DEV__) {
							console.warn("[useSectionChat] Participants query error:", participantsError);
						}
						
						// If participants query didn't work or found nothing, try direct conversations query
						if (!resolvedConvId) {
							if (__DEV__) console.log("[useSectionChat] Trying direct conversations query for group_section");
							const { data: convsData, error: convsError } = await (supabase as any)
								.from("conversations")
							...
							if (!convsError && convsData && convsData.length > 0) {
								...
								for (const conv of convsData) {
									...
									if (msgCheck && msgCheck.length > 0) {
										...
										if (__DEV__) console.log("[useSectionChat] Found section conversation with messages:", resolvedConvId);
										break;
									}
								}
								...
								if (!resolvedConvId && convsData.length > 0) {
									...
									if (__DEV__) console.log("[useSectionChat] Using first group_section conversation:", resolvedConvId);
								}
							}
						}
					} catch (e) {
						if (__DEV__) console.warn("[useSectionChat] Supabase conversation lookup error:", e);
					}
				}
				...
				if (!resolvedConvId) {
					...
					if (__DEV__) console.warn("[useSectionChat] Using section_id as conversation_id (may not match):", resolvedConvId);
				}
				...
				if (supabase && resolvedConvId && !cancelled) {
					...
					if (messagesError) {
						if (__DEV__) console.warn("[useSectionChat] Join failed, trying simple query:", messagesError.message, messagesError);
						...
						if (!simpleError && simpleData && simpleData.length > 0) {
							...
							const senderIds = [...new Set(simpleData.map((m: any) => m.sender_id).filter(Boolean))];
							if (__DEV__) console.log("[useSectionChat] Fetching users for sender IDs:", senderIds);
							...
							if (__DEV__) console.log("[useSectionChat] Users query result:", { 
								hasData: !!usersData, 
							...
							if (__DEV__) console.warn("[useSectionChat] No messages found for conversation:", resolvedConvId);
						}
					} else if (messagesData) {
						if (__DEV__) console.log("[useSectionChat] Join query succeeded, messagesData:", messagesData.length, messagesData.slice(0, 2));
						...
						if (missingSenderIds.length > 0) {
							...
							if (participantsMap.size > 0) {
								...
								participantsMapRef.current = new Map(participantsMap);
								if (__DEV__) {
									console.log("[useSectionChat] Found", foundFromMap, "sender names from participants map");
								}
							}
							...
							if (stillMissing.length > 0) {
								if (__DEV__) console.log("[useSectionChat] Fetching remaining sender names from Supabase for IDs:", stillMissing);
								const { data: usersData, error: usersError } = await (supabase as any)
									.from("users")
									.select("id, full_name, email")
								...
								if (!usersError && usersData && usersData.length > 0) {
									...
									normalized = normalized.map((m: any) => ({
										...m,
										sender_name: m.sender_name || usersMap.get(m.sender_id)?.full_name || null,
									})) as ChatMessage[];
									if (__DEV__) console.log("[useSectionChat] Fetched sender names for", usersData.length, "users from Supabase");
								} else if (usersError && __DEV__) {
									console.warn("[useSectionChat] Users query error:", usersError);
								} else if (__DEV__) {
									console.warn("[useSectionChat] Users query returned empty - RLS may be blocking");
								}
							}
						}
	...
	}, [user, user?.student?.section_id, chatServiceBase]);
	...
	useEffect(() => {
		...
		if (!supabase) {
			console.warn("[useSectionChat] Supabase not available - realtime disabled");
			return;
		}

		if (__DEV__) console.log("[useSectionChat] Subscribing realtime for conv:", conversationId);
			...
			.on("postgres_changes", {
				...
				if (msg.sender_id && !msg.sender_name) {
					...
					if (participant) {
						...
						if (__DEV__) console.log("[useSectionChat] Using sender name from participants map:", msg.sender_name);
					} else if (supabase) {
						...
						if (userData && !userError) {
							...
							if (__DEV__) console.log("[useSectionChat] Fetched sender name for realtime message:", msg.sender_name);
						} else if (__DEV__) {
							console.warn("[useSectionChat] Failed to fetch sender name:", userError);
						}
					}
				}
				
				if (__DEV__) console.log("[useSectionChat] Realtime message:", msg.id, msg.sender_name);
				setMessages(prev => {
					// Avoid duplicates
	...
	const unreadCount = useMemo(() => calculateUnread(messages, lastReadAt), [messages, lastReadAt]);
	...
	const sendMessage = useCallback(async (content: string) => {
		...
		if (chatServiceBase) {
			const url = `${chatServiceBase.replace(/\/$/, '')}/api/v1/chat/messages`;
			if (__DEV__) console.log("[useSectionChat] POST", url, { conversationId });
			
			const authToken = await getSupabaseAuthToken(tokens);
			const headers: HeadersInit = { "Content-Type": "application/json" };
			if (authToken) {
				headers["Authorization"] = `Bearer ${authToken}`;
				if (__DEV__) console.log("[useSectionChat] Including auth token in POST request");
			} else {
				if (__DEV__) console.warn("[useSectionChat] No auth token available for POST request");
			}
